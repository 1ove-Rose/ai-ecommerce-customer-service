...
class LongTermMemory:
    ...
    def __init__(
        ...
        self.embedding_backend = (embedding_backend or os.getenv("EMBEDDING_BACKEND", "sentence_transformers")).lower()
        self.embedding_model_name = embedding_model or os.getenv(
            "LOCAL_EMBEDDING_MODEL",
            "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2",
        )
        self._embedder: Any = None
        self.embedding_dim = embedding_dim or self._resolve_embedding_dim()
        self._documents: list[dict[str, Any]] = []
        self._index = None
        self._init_index()

    def _resolve_embedding_dim(self) -> int:
        """确定向量维度。优先使用本地 sentence-transformers 模型。"""
        if self.embedding_backend == "sentence_transformers" and SentenceTransformer is not None:
            try:
                self._embedder = SentenceTransformer(self.embedding_model_name)
                return int(self._embedder.get_sentence_embedding_dimension())
            except Exception:
                self._embedder = None

        return int(os.getenv("HASH_EMBEDDING_DIM", "1536"))

    def _embed_text(self, text: str) -> np.ndarray:
        """文本向量化：本地模型优先，未安装或加载失败时使用轻量关键词向量。"""
    ...
    def _init_index(self):
        ...
        if self.index_path.exists():
            try:
                ...
                if getattr(self._index, "d", self.embedding_dim) != self.embedding_dim:
    ...
    def _simple_embedding(self, text: str) -> np.ndarray:
        ...
        这里不再使用“整段文本 hash 后随机向量”，而是做一个稳定的
        hashed bag-of-words 表示：相同词项稳定落到同一维度，中文场景
        额外加入字符 bigram，保证没有外部 embedding 服务时仍能按关键词
        和短语重叠进行可解释检索。
        """
    ...
    def load_knowledge_base(self, kb_dir: str) -> int:
        """从目录批量加载知识库文档，支持 .md 和 .txt。"""
        kb_path = Path(kb_dir)
        if not kb_path.exists():
            return 0

        count = 0
        for file_path in sorted([*kb_path.glob("**/*.md"), *kb_path.glob("**/*.txt")]):
            ...
            for index, chunk in enumerate(chunks):
                self.add_document(
                    ...
                    metadata={"file": str(file_path), "chunk_index": index},
                )
                count += 1
